...
1.  **Introduction**: Provides context on the importance of robust configuration validation in building a reliable PE intelligence platform.
2.  **Configure Application Settings**: Allows the user (Software Developer/Data Engineer) to interactively input and modify a comprehensive set of application parameters using various Streamlit widgets. This includes application metadata, API settings, LLM provider keys, cost management thresholds, database connections (Snowflake), cloud credentials (AWS), Redis settings, scoring model parameters, and critical dimension weights.
3.  **Validate Configuration**: Upon user action, the application dynamically applies Pydantic's powerful validation logic, including field-level range checks, cross-field summation checks (for dimension weights), and conditional environment-specific rules (e.g., stricter requirements for 'production' settings).
4.  **Review Validation Report**: Presents a clear "Validated Configuration Report" that summarizes the current settings and prominently displays specific error messages for any failed validations, allowing the user to quickly identify and rectify issues. Pre-defined scenarios are available to demonstrate valid and invalid configurations rapidly.

This application highlights how Pydantic integration ensures operational integrity, security, and data consistency for complex platforms.
...
from source import Settings, get_settings_with_prod_validation, clear_env, scenarios
...
`st.session_state` is used to maintain the application's state across user interactions and page navigations.
...
if 'config_inputs' not in st.session_state:
    ...
    # For required fields without a default, provide sensible placeholder values.
    st.session_state.config_inputs = {
        "APP_NAME": "PE Org-AI-R Platform",
        "APP_VERSION": "4.0.0",
        "APP_ENV": "development", # Will be overwritten by app_env if changed in sidebar
        "DEBUG": False,
        "LOG_LEVEL": "INFO",
        "LOG_FORMAT": "json",
        "SECRET_KEY": "default_secret_for_dev_env_testing_0123456789", # Must be >= 32 for prod
        ...
        "SNOWFLAKE_ACCOUNT": "default_snowflake_account", # Placeholder for required field
        "SNOWFLAKE_USER": "default_snowflake_user", # Placeholder for required field
        "SNOWFLAKE_PASSWORD": "default_snowflake_password", # Placeholder for required field
        "SNOWFLAKE_DATABASE": "PE_ORGAIR",
        "SNOWFLAKE_SCHEMA": "PUBLIC",
        "SNOWFLAKE_WAREHOUSE": "default_snowflake_warehouse", # Placeholder for required field
        "SNOWFLAKE_ROLE": "PE_ORGAIR_ROLE",
        "AWS_ACCESS_KEY_ID": "default_aws_key_id", # Placeholder for required field
        "AWS_SECRET_ACCESS_KEY": "default_aws_secret_key", # Placeholder for required field
        "AWS_REGION": "us-east-1",
        "S3_BUCKET": "default_s3_bucket", # Placeholder for required field
    ...
    st.session_state.config_inputs["APP_ENV"] = st.session_state.app_env # Ensure initial match

if 'validation_errors' not in st.session_state:
    st.session_state.validation_errors = {} # Stores {'field_name': 'error message'}

if 'overall_validation_status' not in st.session_state:
    st.session_state.overall_validation_status = "Not Validated" # "Valid", "Invalid"

if 'validated_config_report' not in st.session_state:
    st.session_state.validated_config_report = "" # Detailed report output
...
if 'current_settings_object' not in st.session_state:
    st.session_state.current_settings_object = None # Stores the valid Pydantic Settings object
```

**`st.session_state` Usage (`Read` and `Update` across pages):**

*   **`current_page`**:
    *   **Read**: Determines which main content section is displayed.
    *   **Updated**: Via sidebar `st.selectbox` for page navigation.
*   **`app_env`**:
    *   **Read**: Used as the default for `APP_ENV` input widget on the configuration page and influences production-specific validation rules.
    *   **Updated**: Via sidebar `st.radio` for environment selection. This updates `st.session_state.config_inputs["APP_ENV"]` directly.
*   **`config_inputs`**:
    *   **Read**: Populates all input widgets on the "Configure Application Settings" page. Used to construct `os.environ` for validation.
    *   **Updated**:
        *   Each Streamlit input widget updates its corresponding key-value pair in `st.session_state.config_inputs` directly on change.
        *   The "Load Scenario" button updates `config_inputs` with values from the selected scenario.
*   **`validation_errors`**:
    *   **Read**: Displayed next to relevant input widgets on the "Configure Application Settings" page (using `st.error`). Also shown in the "Validated Configuration Report".
    *   **Updated**: By the "Validate Configuration" button's callback, after parsing `ValidationError` exceptions. Cleared on successful validation or new scenario load.
*   **`overall_validation_status`**:
    *   **Read**: Displayed prominently on the "Validated Configuration Report" page.
    *   **Updated**: By the "Validate Configuration" button's callback, reflecting success or failure. Cleared on new scenario load.
*   **`validated_config_report`**:
    *   **Read**: Displayed in the `st.expander` on the "Validated Configuration Report" page.
    *   **Updated**: By the "Validate Configuration" button's callback, generating a detailed report based on validation outcome. Cleared on new scenario load.
*   **`selected_scenario_name`**:
    *   **Read**: Determines which `scenarios` (from `source.py`) to load.
    *   **Updated**: Via sidebar `st.selectbox` for scenario selection.
*   **`current_settings_object`**:
    *   **Read**: Used for safely displaying masked secret values and other configuration details in the validation report after a successful validation.
    *   **Updated**: Set to the `Settings` object on successful validation, `None` otherwise.

### Application Structure and Flow

```python
# Helper function to parse Pydantic errors into a more usable format for Streamlit
...
# Helper function to generate a report for valid configuration
def generate_valid_report(settings_obj: Settings) -> str:
    ...
    # Convert Settings object to dict, handling SecretStr
    settings_dict = {}
...
# Helper function to generate a report for invalid configuration
...
def load_scenario():
    if st.session_state.selected_scenario_name in scenarios:
        ...
        # Update config_inputs and app_env based on scenario
...
def validate_configuration():
    # 1. Clear existing environment variables (specific to source.py interaction)
    clear_env()

    # 2. Populate os.environ with current config_inputs
    #    Required default environment variables for the Settings class to instantiate
    #    These are added if not explicitly provided in scenario_env_vars or user inputs
    default_required_env_vars = {
        "SECRET_KEY": "default_secret_for_dev_env_testing_01234567890123456789", # Ensure length for production
    ...
    # First, populate from session state inputs
    for key, value in st.session_state.config_inputs.items():
        # Handle bool values for environment variables
        if isinstance(value, bool):
            os.environ[key] = str(value)
        elif value is not None and value != "":
            os.environ[key] = str(value)

    # Then, add any missing required defaults
    for key, value in default_required_env_vars.items():
        if key not in os.environ:
            os.environ[key] = value

    # Ensure APP_ENV in os.environ matches sidebar selection
    os.environ["APP_ENV"] = st.session_state.app_env

    # 3. Clear lru_cache for get_settings_with_prod_validation to pick up new env vars
    get_settings_with_prod_validation.cache_clear()

    # 4. Attempt to load and validate settings
    try:
        ...
        st.session_state.validation_errors = {} # Clear errors on success
        ...
        # 5. Clean up os.environ after validation
        clear_env()

# --- Sidebar Navigation and Controls ---
with st.sidebar:
    st.image("https://www.quantuniversity.com/assets/img/logo.png", width=150) # Placeholder for an image
    ...
    st.session_state.app_env = st.radio(
        ...
        key="sidebar_app_env",
        index=["development", "staging", "production"].index(st.session_state.app_env),
        on_change=lambda: st.session_state.config_inputs.update({"APP_ENV": st.session_state.app_env}) # Update config_inputs directly
    )
    ...
    st.session_state.selected_scenario_name = st.selectbox(
        ...
        list(scenarios.keys()),
        index=list(scenarios.keys()).index(st.session_state.selected_scenario_name),
...
# --- Main Content Area ---
st.title("PE Intelligence Platform: Configuration Validator")
...
if st.session_state.current_page == "Introduction":
    ...
    with st.expander("General Application Settings", expanded=True):
        ...
        # APP_ENV is controlled by sidebar, only display its current value
        ...
        st.session_state.config_inputs["LOG_LEVEL"] = st.selectbox("Log Level", options=["DEBUG", "INFO", "WARNING", "ERROR"], index=["DEBUG", "INFO", "WARNING", "ERROR"].index(st.session_state.config_inputs["LOG_LEVEL"]), key="LOG_LEVEL")
        st.session_state.config_inputs["LOG_FORMAT"] = st.radio("Log Format", options=["json", "console"], index=["json", "console"].index(st.session_state.config_inputs["LOG_FORMAT"]), key="LOG_FORMAT")
    ...
    with st.expander("API Settings"):
        ...
        st.session_state.config_inputs["PARAM_VERSION"] = st.radio("Parameter Version", options=["v1.0", "v2.0"], index=["v1.0", "v2.0"].index(st.session_state.config_inputs["PARAM_VERSION"]), key="PARAM_VERSION")
    ...
    with st.expander("LLM Providers"):
        ...
        # Global model key error if no LLM key provided in production
    ...
    with st.expander("Cost Management"):
        ...
        st.session_state.config_inputs["DAILY_COST_BUDGET_USD"] = st.number_input("Daily Cost Budget (USD)", min_value=0.0, value=st.session_state.config_inputs["DAILY_COST_BUDGET_USD"], format="%.2f", key="DAILY_COST_BUDGET_USD")
        ...
        st.session_state.config_inputs["COST_ALERT_THRESHOLD_PCT"] = st.slider("Cost Alert Threshold (%)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["COST_ALERT_THRESHOLD_PCT"], step=0.01, format="%.2f", help="Percentage of daily budget to trigger an alert (0.0 to 1.0)", key="COST_ALERT_THRESHOLD_PCT")
        ...
        st.session_state.config_inputs["HITL_SCORE_CHANGE_THRESHOLD"] = st.slider("HITL Score Change Threshold", min_value=5.0, max_value=30.0, value=st.session_state.config_inputs["HITL_SCORE_CHANGE_THRESHOLD"], step=0.1, format="%.1f", key="HITL_SCORE_CHANGE_THRESHOLD")
        ...
        st.session_state.config_inputs["HITL_EBITDA_PROJECTION_THRESHOLD"] = st.slider("HITL EBITDA Projection Threshold", min_value=5.0, max_value=25.0, value=st.session_state.config_inputs["HITL_EBITDA_PROJECTION_THRESHOLD"], step=0.1, format="%.1f", key="HITL_EBITDA_PROJECTION_THRESHOLD")
    ...
    with st.expander("Redis Settings"):
        ...
        st.session_state.config_inputs["CACHE_TTL_SECTORS"] = st.number_input("Cache TTL Sectors (seconds)", min_value=1, value=st.session_state.config_inputs["CACHE_TTL_SECTORS"], key="CACHE_TTL_SECTORS")
        st.session_state.config_inputs["CACHE_TTL_SCORES"] = st.number_input("Cache TTL Scores (seconds)", min_value=1, value=st.session_state.config_inputs["CACHE_TTL_SCORES"], key="CACHE_TTL_SCORES")

    # --- Scoring Parameters (v2.0) ---
    with st.expander("Scoring Parameters (v2.0)"):
        st.session_state.config_inputs["ALPHA_VR_WEIGHT"] = st.slider("ALPHA VR Weight", min_value=0.55, max_value=0.70, value=st.session_state.config_inputs["ALPHA_VR_WEIGHT"], step=0.01, format="%.2f", key="ALPHA_VR_WEIGHT")
        if "ALPHA_VR_WEIGHT" in st.session_state.validation_errors:
            st.error(st.session_state.validation_errors["ALPHA_VR_WEIGHT"])
        st.session_state.config_inputs["BETA_SYNERGY_WEIGHT"] = st.slider("BETA Synergy Weight", min_value=0.08, max_value=0.20, value=st.session_state.config_inputs["BETA_SYNERGY_WEIGHT"], step=0.01, format="%.2f", key="BETA_SYNERGY_WEIGHT")
        ...
        st.session_state.config_inputs["LAMBDA_PENALTY"] = st.slider("LAMBDA Penalty", min_value=0.0, max_value=0.50, value=st.session_state.config_inputs["LAMBDA_PENALTY"], step=0.01, format="%.2f", key="LAMBDA_PENALTY")
        if "LAMBDA_PENALTY" in st.session_state.validation_errors:
            st.error(st.session_state.validation_errors["LAMBDA_PENALTY"])
        st.session_state.config_inputs["DELTA_POSITION"] = st.slider("DELTA Position", min_value=0.10, max_value=0.20, value=st.session_state.config_inputs["DELTA_POSITION"], step=0.01, format="%.2f", key="DELTA_POSITION")
    ...
    with st.expander("Dimension Weights", expanded=True):
        ...
        st.session_state.config_inputs["W_DATA_INFRA"] = st.slider("Data Infrastructure Weight (W_DATA_INFRA)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_DATA_INFRA"], step=0.01, format="%.2f", key="W_DATA_INFRA")
        st.session_state.config_inputs["W_AI_GOVERNANCE"] = st.slider("AI Governance Weight (W_AI_GOVERNANCE)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_AI_GOVERNANCE"], step=0.01, format="%.2f", key="W_AI_GOVERNANCE")
        st.session_state.config_inputs["W_TECH_STACK"] = st.slider("Tech Stack Weight (W_TECH_STACK)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_TECH_STACK"], step=0.01, format="%.2f", key="W_TECH_STACK")
        st.session_state.config_inputs["W_TALENT"] = st.slider("Talent Weight (W_TALENT)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_TALENT"], step=0.01, format="%.2f", key="W_TALENT")
        st.session_state.config_inputs["W_LEADERSHIP"] = st.slider("Leadership Weight (W_LEADERSHIP)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_LEADERSHIP"], step=0.01, format="%.2f", key="W_LEADERSHIP")
        st.session_state.config_inputs["W_USE_CASES"] = st.slider("Use Cases Weight (W_USE_CASES)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_USE_CASES"], step=0.01, format="%.2f", key="W_USE_CASES")
        st.session_state.config_inputs["W_CULTURE"] = st.slider("Culture Weight (W_CULTURE)", min_value=0.0, max_value=1.0, value=st.session_state.config_inputs["W_CULTURE"], step=0.01, format="%.2f", key="W_CULTURE")
