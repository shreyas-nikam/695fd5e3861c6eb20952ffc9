...
# Streamlit Application Specification: PE Intelligence Platform Configuration Lab

## 1. Application Overview

### Purpose of the Application

The "PE Intelligence Platform Configuration Lab" is a Streamlit application designed to guide Software Developers and Data Engineers through the essential steps of setting up a robust, production-ready configuration system for the PE Org-AI-R Platform. Using Pydantic v2, the application demonstrates how to define, validate, and manage application settings, focusing on preventing common configuration-related bugs and enhancing application security and stability. Users will learn to implement various validation techniques, including field-level, cross-field, and environment-specific checks, simulating real-world deployment scenarios to ensure operational integrity and confidence in their configurations.

### High-Level Story Flow of the Application

The user embarks on a learning journey structured into sequential lab sections, accessible via a sidebar navigation.

1.  **Introduction:** The journey begins with an overview of the challenges posed by misconfiguration in production AI systems and introduces Pydantic v2 as a solution for building a robust configuration system.
2.  **Initial Setup:** Users learn to define the core application settings using Pydantic's `BaseSettings` and `SecretStr` for sensitive data, mimicking the initial setup of a new service. They interact by loading default settings to see the initial configuration.
3.  **Field-Level Validation:** The application then guides the user to apply field-level constraints (`ge`, `le`) to operational parameters like API rate limits and cost budgets. Users input values and observe how Pydantic automatically catches out-of-range inputs, demonstrating early error detection.
4.  **Cross-Field Validation (Scoring Weights):** The user progresses to implementing complex business logic, specifically ensuring that a set of critical investment scoring dimension weights sums to exactly 1.0. They input various weights and see how a Pydantic `@model_validator` enforces this cross-field rule, preventing skewed analytical outcomes.
5.  **Environment-Specific Validation (Production):** The focus shifts to production readiness, where users learn to enforce strict security and operational policies (e.g., `DEBUG` mode off, minimum `SECRET_KEY` length, required API keys) only when the application is in a "production" environment. They configure settings and validate them under production rules.
6.  **Configuration Simulation & Troubleshooting:** Finally, the user experiences a "Validated Configuration Report." The application simulates multiple configuration scenarios, intentionally introducing errors, and demonstrates how Pydantic's comprehensive error reporting helps in catching issues early, preventing costly runtime failures, and providing clear troubleshooting guidance for common mistakes.

Throughout these steps, `st.session_state` is utilized to maintain the user's progress and display validation results, providing a continuous and interactive learning experience.
...
The `st.session_state` dictionary will be used to manage the application's state across user interactions and page navigations.
...
# These will be set and cleared around each function call for isolation.
...
def _clear_env_vars():
    ...
    # This list should match prefixes used for Pydantic settings.
...
**Usage (Update/Read):**
Each page will update and read specific `st.session_state` keys to reflect user interaction outcomes and ensure persistence across page switches.

### Application Structure and Flow

The application will use a sidebar for navigation to simulate a multi-page experience.

```python
st.set_page_config(layout="wide", page_title="PE Platform Configuration Lab")
...
st.title("PE Intelligence Platform: Robust Configuration with Pydantic v2")

if st.session_state.current_page == "Introduction":
    # Content for Introduction Page
    st.markdown(f"## Introduction: Safeguarding the PE Intelligence Platform")
    st.markdown(f"") # Separator for better rendering of markdown
    ...
    # Content for Initial Setup Page
    ...
    if st.button("Load Default Application Settings"):
        ...
        get_settings.cache_clear() # Clear cache for new env vars to be picked up
        try:
    ...
    # Content for Field-Level Validation Page
    ...
    # Content for Cross-Field Validation Page
    ...
    # Content for Environment-Specific Validation Page
    ...
    # Content for Configuration Simulation & Troubleshooting Page
    ...
    if st.button("Run All Configuration Scenarios"):
        ...
        for name, env_vars_dict in scenarios.items():
            ...
            # Capture stdout for function that prints results
            f = io.StringIO()
            with redirect_stdout(f):
                # Call the function directly from source.py
                # Note: load_scenario_settings internally calls _clear_env_vars,
                # _set_env_vars (implicitly by setting os.environ), and cache_clear.
